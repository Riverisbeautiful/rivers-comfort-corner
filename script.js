document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadEntries() {
        ...
        if (savedEntries.length === 0) {
            journalHistoryList.innerHTML = `<p class="empty-msg">No journal entries saved yet.</p>`;
            return;
        }
        ...
        savedEntries.forEach(entry => {
            ...
            card.innerHTML = `
                <div class="history-date">${entry.date} (Mood: ${entry.mood})</div>
                <p class="history-text">${entry.text}</p>
            `;
            journalHistoryList.prepend(card);
        });
    }
